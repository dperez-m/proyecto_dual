                ...
                <div className="flex flex-col items-center w-full">
                    ...
                    <div className="flex flex-col sm:flex-row justify-around items-start px-4 py-6 overflow-y-auto h-[calc(75vh-48px)] w-full z-10">
                        {selectedCars.map((car, index) => {
                            ...
                            return (
                                <div key={car.id} className="flex flex-col items-center w-full max-w-sm text-sm text-center">
                                    <h2 className="text-lg font-semibold mb-2">{car.brand} {car.model}</h2>
                                    ...
                                    <ul className={`leading-6 text-lg space-y-1 w-full max-w-[420px] mx-auto ${index === 0 ? 'text-left' : 'text-right'}`}>
                                        <li><strong>Tipo: </strong>
                                            {car.category}</li>
                                        <li><strong>Precio: </strong>
                                            {car.price.toLocaleString()} €</li>
                                        <li><strong>Batería: </strong>
                                            {car.battery_range} km</li>
                                        <li><strong>Potencia: </strong>
                                            {car.power} CV</li>
                                        {car.drivetrain && <li><strong>Tracción: </strong>
                                            {car.drivetrain}</li>}
                                        {car.seats && <li><strong>Plazas: </strong>
                                            {car.seats}</li>}
                                        {car.trunk_capacity && <li><strong>Maletero: </strong>
                                            {car.trunk_capacity} L</li>}
                                        {car.fast_charging_power && <li><strong>
                                            Potencia carga rápida: </strong>
                                            {car.fast_charging_power} kW</li>}
                                    </ul>
                                </div>
                            );
                        })}
